...
Par suite $\indice{H}{BF}(p)=\dfrac{F(p) \indice{H}{cor}(p)}{1+F(p) \indice{H}{cor}(p)}$ soit
 $\indice{H}{BF}(p)=\dfrac{\dfrac{H_1(p)H_2(p)H_3(p)}{1+H_1(p)H_2(p)H_3(p)}}{1+\dfrac{H_1(p)H_2(p)H_3(p)}{1+H_1(p)H_2(p)H_3(p)}} = $
...
\question{À partir des courbes suivantes, proposer une valeur de coefficient d'amortissement et de pulsation propre.}
...
\\
\question{Tracer le diagrame de Bode de la fonction de transfert $G(p)=\dfrac{K_i\left(1+T_i p\right)}{T_i p\left(1+\tau p^2\right)}$.}
