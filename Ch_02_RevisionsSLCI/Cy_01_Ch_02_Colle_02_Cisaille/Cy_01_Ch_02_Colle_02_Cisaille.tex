% "{'chapitre':'slci_bode','classe':('PSI'),'type':('colle'),'titre':'Cisaille à découpe au vol', 'source':'P. Dubois et C. Gamelon','comp':['B2-04',],'corrige':True}"
%\setchapterimage{fig_00}
\chapter*{Colle \arabic{cptColle} \\ 
Cisaille à découpe au vol -- \ifprof Corrigé \else Sujet \fi}

\addcontentsline{toc}{section}{Colle \arabic{cptColle} : Cisaille à découpe au vol -- \ifprof Corrigé \else Sujet \fi}
...
\marginnote{D'après P. Dubois, C. Gamelon..}
...
\marginnote{Rappels : $\mathcal{L}\left[a\right]=\dfrac{a}{p}$, $\mathcal{L}\left[at\right]=\dfrac{a}{p^2}$ et $\mathcal{L}\left[e^{-at}\right]=\dfrac{1}{p+a}$. }
...
\marginnote{
$X(p)$ est la transformée de Laplace du déplacement $x(t)$ et $Q(p)$ celle du débit $q(t)$).}
...
%\subsection*{Fonction de transfert de l'ensemble vérin et charge}
