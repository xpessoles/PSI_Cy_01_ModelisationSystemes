...
\addcontentsline{toc}{section}{Application \arabic{cptApplication} : Réponses fréquentielles -- \ifprof Corrigé \else Sujet \fi}
...
\marginnote{On a  $A = \SI{100}{g.m.s^{-2}.V^{-1}}$, $\tau=\SI{0,2}{s}$ et $S = 10\cdot 10^{-3}/g V/(m/s^2)$ où $g$ est l'accélération de pesanteur. $E(p)$ est la transformée de Laplace de $e(t)$ la tension de consigne de cet asservissement et $\Gamma_b(p)$ la transformée de l'accélération $\gamma_b(t)$.}
